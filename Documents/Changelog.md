...
| # | Name | Date | Notes |
|---|------|------|-------|
| 48 | Catalogue Row-Cap Sweep + IndexNow | Aug 2, 2026 | The sitemap fix (47) was one symptom of a systemic bug: PostgREST's 1000-row cap was silently truncating six reads that enumerate the whole catalogue, all of which crossed the line the day it passed 1,000 products. `getProductStats()` fed `{total}` into editorial copy, so the site stated **1,000 products as fact**; `getSeasons()`/`getSeasonNumbers()` read one row per product ordered by season descending, so /seasons lost **seasons 1–6**; `getProductSlugs()` capped `generateStaticParams` (static pages 1,055 → 1,656); category counts were understated; and `submit-indexnow.ts` would have submitted only 1,000 of 1,595. `selectAll()` now lives in `src/lib/supabase/select-all.ts` as the single implementation. Submitted all 1,609 URLs to IndexNow (Bing/Yandex — Google doesn't support it) |
| 47 | Sitemap Row-Cap Fix | Aug 2, 2026 | PostgREST caps an unbounded select at 1000 rows, so the sitemap silently truncated once the catalogue crossed it: 980 of 1,595 products, and only 12 of 17 seasons because the seasons query reads one row per product and never reached season 6. About 615 URLs were missing — including every product in seasons 1–6, immediately after a backfill meant to make them discoverable. `selectAll()` now pages with `.range()` across products, seasons, and episodes. Same failure mode as milestone 31's `generateStaticParams` cap. Verified on a clean build: 2,098 URLs — 1,595 products, 47 sharks, 17 seasons, 377 episodes |
| 46 | Dad Strength Brewing Deal Correction | Aug 2, 2026 | `deal_equity` held 15% — the equity the Sharks *offered*. The founders countered at $300K for 12% (4% each to Greiner, Herjavec, Williams) and that is what was accepted; verified against Mashed and Parade. The record contradicted itself, since the per-shark rows already said 4% each. Corrected to 12%; the generated `deal_valuation` recomputed to $2.5M. The narrative already described the negotiation accurately, so the scheduled refresh the deal-change trigger queued was cleared rather than spending a regeneration on correct copy. Swept every deal with equity ≥60% or ≤0.5% for the same error and found none — Groovebook's 80%, Readerest's 65%, and the Hugo's Amazing Tape and Turbobaster 100% buyouts are all real |
| 45 | Shark Record Cleanup + One Prompt Per Job | Aug 2, 2026 | Merged the last duplicate shark records — `Daniel` and `Daniel Lubetsky` into Daniel Lubetzky, `Rashaun L. Williams` into Rashaun Williams — after verifying both attributions against sources (McMiller is Lubetzky's deal; Dad Strength Brewing is a three-shark deal). Old slugs 308-redirect. 47 sharks, no duplicates. Also removed the duplicated prompts that let the shark bugs survive in the first place: `daily-enrich-pending.ts` was a 347-line reimplementation of the cron's own `enrichPendingDeals()` and is now a 110-line wrapper over it, and `enrich-product.ts` imports the shared schema and prompt instead of a local copy. Found `DEAL_SEARCH_PROMPT` — the prompt the *daily cron* actually uses — still carried the offer≠deal bug after milestone 44 fixed only the full-enrichment one; fixed too |
...
- **Sharks**: 47, including 2 retired; every recorded deal has its investor linked and all duplicate records are merged, with 308 redirects for the retired slugs
